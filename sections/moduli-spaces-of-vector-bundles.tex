...
We now want to show an equivalence of categories
\begin{equation} \label{equivalence bun pt1}
\Bun_n^d(X) \cong \Bun_{(0, \dots, 0, n, 0, \dots, 0 )}^0(\mc{X}),
\end{equation}
where on the right hand side, the $n$ is in the position $-d (\mod n)$. In order to simplify the notation, let us denote this multiplicity vector by $n_{-d}$. Similarly, we will denote $(n, 0, \dots, 0)$ by $n_0$.
% POSSIBLE ALTERNATIVE SYMBOL: $n \mathbb{1}_{-d}$

As a first step, let us show that the pullback and pushforward functors define equivalences $\Bun_n^d(X) \cong \Bun_{n_0}^d(\mc{X}).$
