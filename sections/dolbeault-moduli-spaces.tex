...
We now want to extend the results of the previous section to the Dolbeault moduli spaces and find an equivalence
\begin{equation} \label{equivalence dol}
\mc{M}_{n, Dol}^d(X) \cong \mc{M}_{n_{-d}, Dol}^0(\mc{X}).
\end{equation}

As before, we will first extend \eqref{equivalence bun} to the moduli spaces of Higgs bundles and then \eqref{equivalence dol} will follow.

\zcref{omega mc(X)} gave an isomorphism
\begin{equation}
\Omega_{\mc{X}} \cong \pi^* \Omega_X \otimes \O(\tfrac{1}{n}p)^{\otimes n-1}.
\end{equation}
Notice moreover that, under the above isomorphism, we can give an an explicit expression for the canonical morphism
\begin{equation} \label{eq: canonical morphism cotangent}
\pi^* \Omega_X \to \Omega_{\mc{X}} \cong \pi^*\Omega_X \otimes \O(\tfrac{n-1}{n}p).
\end{equation}
Namely, \eqref{eq: canonical morphism cotangent} has the form $n \id_{\pi^* \Omega_X} \otimes s^{n-1}$, where $s$ is the tautological section of $\O(\tfrac{1}{n}p)$.\\
Indeed, the coarse moduli map $\pi \colon \mc{X} \to X $ locally around $p$ has the following form
\[\begin{tikzcd}
	{\Ab^1 = \Spec k[x]} \\
	{\mc{X} = [\Ab^1 / \mu_n]} \\
...
\end{align}
Since $q'$ is étale, it induces isomorphisms on the cotangent sheaves. Moreover, the section $x$ corresponds via $q'$ to the tautological section $s$ of $\O(\tfrac{1}{n}p)$. In conclusion, we get that the canonical morphism $\pi^* \Omega_X \to \Omega_{\mc{X}} \cong \pi^*\Omega_X \otimes \O(\tfrac{n-1}{n}p)$ ha the indicated form.

Now we can use this morphism to extend the pullback functor $\pi^*$ to the Higgs bundles. In particular, we need to lift the Higgs field.
Define
\begin{align}
...
We now need to show that the functors $(\pi^*, \pi^{\#})$ and $(\pi_*, \pi_{\#})$ are inverse to each other. 
At the level of vector bundles this was shown in \zcref{section: moduli spaces of vector bundles}. We need to show that the same holds for the Higgs fields. Indeed, computing the two compositions we get
\begin{gather}
...
In conclusion, we get an equivalence
\begin{equation}
\begin{tikzcd}
